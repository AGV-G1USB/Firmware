...
**     Date/Time   : 2018-04-30, 14:14, # CodeGen: 7
**     Abstract    :
...
**             Xtal ticks              : 33         21
**             microseconds            : 1000       650
**             milliseconds            : 1          1
**             seconds (real)          : 0.000999987125 0.000649988651
**
**     Contents    :
...
static void SetRatio(void)
{
  if (ActualRatio == 0xFFFFU) {        /* Duty = 100%? */
    ...
    TPM3C0V = (word)(((0x4189UL * (dword)ActualRatio)  + 0x8000UL) >> 0x10U); /* Calculate new compare value according to the given ratio */
  }
}
...
byte PWM1_SetDutyMS(word Time)
{
  ...
  PE_Timer_LngMul((dword)Time, 0x010000D8UL, &rtval); /* Multiply given value and High speed CPU mode coefficient */
  if (PE_Timer_LngHi1(rtval[0], rtval[1], &ActualRatio)) { /* Is the result greater or equal than 65536 ? */
    ActualRatio = 0xFFFFU;             /* If yes then use maximal possible value */
  }
  SetRatio();                          /* Calculate and set up new appropriate values of the compare and modulo registers */
  return ERR_OK;                       /* OK */
...
void PWM1_Init(void)
{
  ...
  /* TPM3C0SC: CH0F=0,CH0IE=0,MS0B=1,MS0A=1,ELS0B=1,ELS0A=0,??=0,??=0 */
  setReg8(TPM3C0SC, 0x38U);            /* Set up PWM mode with output signal level high */ 
  ActualRatio = 0xA667U;               /* Store initial value of the ratio */
  /* TPM3MOD: BIT15=0,BIT14=1,BIT13=0,BIT12=0,BIT11=0,BIT10=0,BIT9=0,BIT8=1,BIT7=1,BIT6=0,BIT5=0,BIT4=0,BIT3=1,BIT2=0,BIT1=0,BIT0=0 */
  setReg16(TPM3MOD, 0x4188U);          /* Set modulo register */ 
